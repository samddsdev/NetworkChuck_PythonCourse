...
def clear_screen():
    print(
        "\033[H\033[2J\n", end=""
    )  # \033[H moves the cursor to the top left corner. \033[2J clears the terminal screen.
...
local_time = (
    time.localtime()
)  # Presents local time in human readable format. Used to extract .tm_hour value.
...
try:  # "Listens" for Ctrl+C input to end program cleanly.
    ...
    )  # capitalises first letter of name and removes spaces.
    ...
    if name == "Ben":
        ...
        if (
            evil_status == "yes"
        ):  # denies evil Ben entry to coffee shop and ends program
            clear_screen()
    ...
    while True:
        try:
            ...
            if (
                1 <= coffee_selection <= len(coffee_menu)
            ):  # If 1 is less than or equal to the number entered and less than or equal to the length of the coffee list.
                break  # Valid input, continue with program.
            else:
                print(
                    f"\033[31mInvalid input:\033[0m Please type a number between 1 and  {len(coffee_menu)}.\n"
                )
